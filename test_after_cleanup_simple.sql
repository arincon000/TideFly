-- Simple test script to verify everything works after cleanup
...
-- Step 5: Verify no broken dependencies - Views
SELECT 'Views that reference min_nights/max_nights:' as info;
SELECT schemaname, viewname
FROM pg_views 
WHERE definition LIKE '%min_nights%' OR definition LIKE '%max_nights%';

-- Step 6: Verify no broken dependencies - Functions
SELECT 'Functions that reference min_nights/max_nights:' as info;
SELECT 
